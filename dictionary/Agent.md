一個配備了[工具](./Tool.md)（Tools）、[系統提示詞](./System%20prompt.md)（System Prompt）和[上下文視窗](./Context%20window.md)（Context Window）的[模型](./Model.md)（Model），透過[對話輪次](./Turn.md)與使用者互動。*Claude Code 是代理人。Cursor 是代理人。Claude.ai 是代理人。* 代理人是你實際與之對話的存在——是運作中的模型，針對特定目的進行配置。
